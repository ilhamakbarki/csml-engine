...
#[tracing::instrument(name = "db.message.add_bulk", skip_all, fields(otel.kind = "internal", db.layer = "dispatch", db.operation = "insert", bot_id = crate::utils::trunc(&data.client.bot_id), channel_id = crate::utils::trunc(&data.client.channel_id), direction = crate::utils::trunc(direction), db.batch_size = msgs.len() as i64))]
...
#[tracing::instrument(name = "db.message.list", skip_all, fields(otel.kind = "internal", db.layer = "dispatch", db.operation = "select", bot_id = crate::utils::trunc(&client.bot_id), channel_id = crate::utils::trunc(&client.channel_id), db.limit = limit.unwrap_or(0) as i64, db_paginated = pagination_key.is_some(), db_from_date = ?from_date, db_to_date = ?to_date))]
